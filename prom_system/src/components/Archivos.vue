<template>
  ...
  <div class="container mt-4 div-selector tit-select">
      ...
      <div class="selector">
        ...
        <select id="menu" v-model="idSeleccionado" class="form-select form-select-sm select-files w-50 w-sm-100" aria-label=".form-select-lg example">
      <option selected disabled>Open this select menu</option>
    <option v-for="c in lasCampanas.filter(campana => campana.activa === '1' || campana.activa === '0')" :key="c.id" :value="c.id">
        {{ c.tipo_de_lanzamiento }} - {{ c.nombre_lanzamiento }} by {{ c.artista }}
    </option>
...
<div class="container mt-4 cont-button">
  ...
  <button :disabled= "!nombreLanzamiento" class="btn-edark" @click="agregarArchivo"><i class="bi bi-file-earmark-plus"></i> Agregar archivo</button>
</div>
...
<div class="container mt-4 div-selector los-archivos">
  ...
  <div class="idioma-box">{{ a.idioma || '-' }}</div>
          ...
          <td>
            <div>
              <button type="button" class="btn btn-econf" @click="editarArchivo(a)"><i class="bi bi-pencil-square"></i> UPDATE</button>
              <button type="button" class="btn btn-edelete" @click="abrirModalEliminar(a.id)" ><i class="bi bi-trash3"></i> DELETE</button>
            </div>
          </td>
        </tr>
      </tbody>
...
<div class="modal fade" id="modalConfirmarEliminar" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content text-dark">
      ...
      <div class="form-actions d-flex justify-content-end">
        <button class="btn-edark" 
        data-bs-dismiss="modal"
        >Cancelar</button>

        <button class="btn-edelete" @click="confirmarEliminacion">Eliminar</button>
      </div>
    </div>
  </div>
</div>
...
#modalEditar select:focus {
  ...
  .tit-select {
 background: linear-gradient(135deg, var(--dark-echo) 65%, var(--medium-echo) 100%);
 box-shadow: 
...
.tit-select::before {
 ...
 width: 100%;
 height: 100%;
 background: linear-gradient(90deg, transparent, rgba(203, 213, 223, 0.1), transparent);
 transition: left 0.6s ease;
}

.tit-select:hover {
 background: linear-gradient(135deg, var(--dark-echo) 45%, var(--medium-echo) 85%, var(--light-echo) 100%);
 box-shadow: 
...
tr{
  border-bottom: .1rem solid var(--medium-echo);
  background-color: var(--dark-echo);
  color: var(--light-echo);
}
