<template>
      ...
      <div class="col-md-9 container-bandeja">
        ...
        <div class="div-filtros">
          <div class="d-flex gap-1 align-items-center">
            ...
            <div class="ms-auto">
              <input type="text" class="form-control form-control-sm searcher-echo" placeholder="Buscar..." style="width: 250px;" />
            </div>
          </div>
        </div>
          ...
          <div class="card-body p-0">
            ...
            <div v-else class="list-group list-group-flush scroll-echo">
              <div v-for="mensaje in mensajesRecibidos" :key="mensaje.id_contacto" 
                ...
                <div class="d-flex w-100 div-inbox div_msg" 
                     @click="abrirMensaje(mensaje)">
                  <div class="flex-grow-1">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                      <h6 class="mb-0">{{ mensaje.nombre }}</h6>
                      <small class="text-muted text-fecha">{{ tiempoRelativo(mensaje.fecha) }}</small>
                    </div>
                    <p class="mb-1 fw-bold custom-dark">{{ mensaje.asunto }}</p>
                    <p class="mb-0 text-muted custom-muted mensaje-bandeja" v-html="mensaje.mensaje"></p>
                    <div class="d-flex justify-content-between align-items-center mt-2">
                      <small class="text custom-dark">{{ mensaje.idioma }}</small>
                      
                      <span v-if="mensaje.estado === null" class="badge estado pendiente">
                        <i class="fas fa-hourglass-half me-1"></i> PENDIENTE
                      </span>

                      <span v-else-if="mensaje.estado === 0" class="badge estado programado">
                        <i class="fas fa-clock me-1"></i> PROGRAMADO
                      </span>

                      <span v-else-if="mensaje.estado === 1" class="badge estado respondido">
                        <i class="fas fa-reply me-1"></i> RESPONDIDO
                      </span>

                      <span v-else-if="mensaje.estado === 2" class="badge estado requiere-revision">
  ...
  <div class="modal fade inbox" id="mensajeProg" tabindex="-1" aria-labelledby="mensajeProgLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" style="max-height: 90vh; display: flex; flex-direction: column;">
      <div class="modal-content modal-content-echo" style="display: flex; flex-direction: column; max-height: 100%;">
        
        <!-- header buttons -->
        <div class="modal-header" style="flex-shrink: 0;">
          <div>
            <h5 class="modal-title" id="mensajeProgLabel">Asunto: {{ mensajeSeleccionado?.asunto || '' }}</h5>
        ...
        <div class="scroll-echo" style="flex: 1; overflow-y: auto;">
          <div class="modal-body">
            <div v-if="mensajeSeleccionado">
              ...
              <div class="mb-3">
                ...
                <div class="mt-2 p-3 bg-light rounded" v-html="mensajeSeleccionado.mensaje"></div>
              </div>
            </div>
            <div v-else>
              <p class="text-center text-muted">Cargando información del mensaje...</p>
            </div>
          </div>
          ...
          <div v-if="programando" class="respuesta p-3 border-top">
            <div class="mb-3">
              <label class="form-label fw-bold">Vista previa del mensaje:</label>
              <div class="mensaje-preview p-3 bg-light rounded border">
                ...
                <div style="white-space: pre-wrap;" v-html="mensajeCompleto"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="modal-footer" style="flex-shrink: 0;">
          <div class="d-flex me-auto div-toggle ">
            ...
            <div class="btn-link">
              <button class="btn btn-eclose" :disabled="!programando" @click="insertarArchivo('audio')">Audio</button>
              <button class="btn btn-eclose" :disabled="!programando" @click="insertarArchivo('ficha')">Rider</button>
              <button class="btn btn-eclose" :disabled="!programando" @click="insertarArchivo('video')">Vídeo</button>
            </div>
          </div>

          <button type="button" class="btn btn-eclose" data-bs-dismiss="modal">Cerrar</button>
          <button type="button" class="btn btn-econf" v-if="mensajeSeleccionado"
...
aria-labelledby="iaModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>¿Estás seguro que deseas analizar los emails con IA? Este proceso puede tomar varios minutos.</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-eclose" data-bs-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-econf" @click="ejecutarAnalisisIA">Analizar</button>
      </div>
    </div>
  </div>
</div>
...
<!-- modal confirmacion responder a toda la campaña -->
    <div class="modal fade" id="responderTodos" tabindex="-1" 
aria-labelledby="responderTodosLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="responderTodosLabel">Confirmar respuestas</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <p>¿Estás seguro que deseas responder todos los emails de esta campaña? Este proceso puede tomar varios minutos.</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-eclose" data-bs-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-econf" @click="ejecutarRespuestas">Responder</button>
      </div>
    </div>
  </div>
</div>
...
<!-- modal confirmacion resetear analisis -->
    <div class="modal fade" id="resetear" tabindex="-1" 
aria-labelledby="resetearLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <p>¿Estás seguro que deseas resetear todos los análisis de esta campaña?</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-eclose" data-bs-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-econf" @click="ejecutarReset">Resetear</button>
      </div>
    </div>
  </div>
</div>
...
<!-- modal aviso, no hay emails analizadoss -->
    <div class="modal fade" id="noEmails" tabindex="-1" 
aria-labelledby="noEmailsLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
...
const mensajeCompleto = computed(() => {
  ...
  if (enlacesActivos.value.audio && obtenerArchivos('audio')) {
    enlaces.push(`<a href="${obtenerArchivos('audio').url}" target="_blank">${obtenerArchivos('audio').url}</a>`);
  }
  
  if (enlacesActivos.value.video && obtenerArchivos('video')) {
    enlaces.push(`<a href="${obtenerArchivos('video').url}" target="_blank">${obtenerArchivos('video').url}</a>`);
  }
  
  if (enlacesActivos.value.ficha && obtenerArchivos('ficha')) {
    enlaces.push(`<a href="${obtenerArchivos('ficha').url}" target="_blank">${obtenerArchivos('ficha').url}</a>`);
  }
...
const obtenerArchivos = (tipo) => {
  ...
  let archivosFiltrados = archivos.value.filter(archivo => 
    archivo.campaña_id === campaniaActiva.value.toString() && archivo.tipo === tipo
  );
  
  if (tipo === 'ficha' && mensajeSeleccionado.value.idioma) {
    const idiomaCodigo = idiomaMap[mensajeSeleccionado.value.idioma] || mensajeSeleccionado.value.idioma;
    archivosFiltrados = archivosFiltrados.filter(archivo => 
      archivo.idioma === idiomaCodigo
    );
  }
...
const insertarArchivo = (tipo) => {
  ...
  if (enlacesActivos.value[tipo] && !archivo) {
    console.warn(`No se encontró archivo de tipo ${tipo} para esta campaña/idioma`);
  ...
  console.log(`${tipo} ${enlacesActivos.value[tipo] ? 'activado' : 'desactivado'}`);
...
watch(mensajeSeleccionado, (nuevoMensaje) => {
  ...
  if (nuevoMensaje) {
    ...
    if (nuevoMensaje.idioma) {
      const idiomaCodigo = idiomaMap[nuevoMensaje.idioma] || nuevoMensaje.idioma;
...
onMounted(() => {
  ...
  fetch('http://localhost/conex-prom-system/api/respuestas_plantillas.php')
    .then(res => res.json())
    .then(json => {
...
function ejecutarReset() {
  ...
  const modal = bootstrap.Modal.getInstance(document.getElementById('resetear'));
  modal.hide();

  fetch('http://localhost/conex-prom-system/api/reset_analisis.php', {
    method: 'POST',
...
function ejecutarAnalisisIA() {
  ...
  fetch(`http://localhost/conex-prom-system/api/ia_update.php?id_campana=${idCampaña}`)
    .then(res => {
      if (!res.ok) {
...
function ejecutarRespuestas() {
  ...
  // Cerrar el modal primero
  const modal = bootstrap.Modal.getInstance(document.getElementById('responderTodos'));
  modal.hide();

  // 👉 Mostrar overlay reactivo
  mostrarOverlay('Respondiendo emails...');

  fetch(`http://localhost/conex-prom-system/api/responder_campana.php?id_campana=${idCampaña}`)
    .then(res => {
    ...
    .then(data => {
      ...
      if (!data.error) {
        ...
        abrirBandeja(campaniaActiva.value); 
         ...
         mostrarToastError("Error: " + (data.message || "No se pudo responder la campaña"));
      }
    })
    .catch(err => {
      console.error("Error en fetch:", err);
...
const abrirBandeja = (id) => {
  ...
  fetch('http://localhost/conex-prom-system/api/tabla_bandeja.php', {
    method: 'POST',
    headers: {
...
const enviarRespuesta = () => {
  ...
  fetch('http://localhost/conex-prom-system/api/update_bandeja.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  .then(data => {
    ...
    if (data.ok) {
      ...
      const modal = bootstrap.Modal.getInstance(document.getElementById('mensajeProg'));
      modal.hide();
      // Actualizar la bandeja para reflejar los cambios
      abrirBandeja(campaniaActiva.value);
    } else {
       mostrarToastError("Error: " + (data.error || "No se pudo programar la respuesta"));
    }
  })
  .catch(err => {
