<template>
  ...
  @click="showSelect = true">
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            <label for="campagneInput" class="form-label subti">Nom de l'artiste</label>
            <input v-model="campagne" type="email" class="form-control" id="exampleFormControlInput1" placeholder="Nom de l'artiste">
            <div class="text-danger small"
               v-if="nombreCampanaExiste">
  ...
  <div class="input-group">
    <select v-model="lancement" class="form-select" style="max-width: 40%;">
      <option disabled value="">Type</option>
      <option value="Album">Album</option>
      <option value="Ep">Ep</option>
      <option value="Single">Single</option>
    </select>
    <input v-model="nomLancement" type="text" class="form-control" placeholder="Ou écris ton type" />
  </div>
</div>
        ...
        <div class="cont-langue w-100 d-flex">
          <div class="ms-auto d-flex flex-row">
            ...
            <button 
              ...
              :class="idiomaSeleccionado === 'EN' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('EN')">
              EN
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'FR' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('FR')">
              FR
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'KO' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('KO')">
              KO
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'UK' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('UK')">
              UK
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'RU' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('RU')">
              RU
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'ES' ? 'btn-primary' : 'btn-outline-secondary'"
              @click="cambiarIdioma('ES')">
              ES
            </button>
            <button 
              class="btn btn-sm m-1 btn-langue" 
              :class="idiomaSeleccionado === 'JP' ? 'btn-primary' : 'btn-outline-secondary'"
...
onMounted(()  =>{
  fetch('http://localhost/conex-prom-system/api/get_email_accounts.php')
  .then(response => {
    if(!response.ok) {
...
onMounted(() => {
  console.log('formularioCompleto ahora esta completo:', formularioCompleto.value);
});
...
function obtenerTraducciones() {
  ...
  const lanzTraducido = tradLanzamiento[tipoLanzamiento]?.[idioma] || tipoLanzamiento;
...
function fueEnviado(item) {
  ...
  console.log(`Verificando contacto ID ${item.id} para campaña ${selectedCamp.value.id}:`, encontrado);
...
//     const response = await fetch(`http://localhost/conex-prom-system/api/credenciales.php?id=${signature.value}`);
...
const seleccionados = datosFiltrados.value.filter(item => checkeds.value.includes(item.id));
	...
	const traduccionesMedia = { // programa o revista
		...
		const tipoLanzamientoActual = showSelect.value && selectedCamp.value ? 
			selectedCamp.value.tipo_de_lanzamiento : 
			lancement.value;
		
		const lanzTraducido = tradLanzamiento[tipoLanzamientoActual]?.[item.secondary_language] || tipoLanzamientoActual;
		...
		const enlace = showSelect.value && selectedCamp.value ? 
  ...
  console.log('datosParaEnviar:', datosParaEnviar);
    ...
    const yaExiste = campEnviadas.value.find( c =>
      c.id_email_account == signature.value && c.activa == 1
    );
...
console.log('enviar a campaña existente (emails para envar):', emailsParaEnviar);
