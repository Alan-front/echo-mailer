...
while ($cuenta = $resCuentas->fetch_assoc()) {
    ...
    $stmtContactos = $con->prepare("
        SELECT DISTINCT 
            ...
            c.id AS id_campana,
            c.nombre_lanzamiento AS nombre_campana,
            c.fecha_creacion
        FROM enviados e
    ...
    while ($row = $resContactos->fetch_assoc()) {
        ...
        // guardar info de campañas que se van a escanear
        if (!isset($campanasUnicas[$row['id_campana']])) {
            $campanasUnicas[$row['id_campana']] = [
                'id_campana' => $row['id_campana'],
                'nombre_campana' => $row['nombre_campana'],
                'id_email_account' => $cuenta['id'],
                'emails_buscados' => 0,
    ...
    if (!$emails) {
        imap_close($inbox);
        // guardar campañas procesadas aunque no haya emails
        foreach ($campanasUnicas as $campana) {
            $campanasEscaneadas[] = $campana;
        }
        continue;
    }
    ...
    foreach ($emails as $email_number) {
        ...
        // extraer fecha ANTES de procesar el mensaje
        ...
        // contar email procesado
        if (isset($campanasUnicas[$idCampanaAsignada])) {
            $campanasUnicas[$idCampanaAsignada]['emails_buscados']++;
        }

        // VERIFICAR DUPLICADO ANTES DE PROCESAR EL MENSAJE COMPLETO
        $stmtCheck = $con->prepare("
            ...
            LIMIT 1
        ");
        $stmtCheck->bind_param("iis", $idCampanaAsignada, $contactoData['id'], $fechaEmailFull);
        $stmtCheck->execute();
        ...
        // SOLO SI ES NUEVO, procesar el mensaje completo
        ...
        ");
        $stmtCount->bind_param("ii", $idCampanaAsignada, $contactoData['id']);
        $stmtCount->execute();
        ...
        // sumar al contador de nuevos
        if (isset($campanasUnicas[$idCampanaAsignada])) {
            $campanasUnicas[$idCampanaAsignada]['nuevos_encontrados']++;
        }

        // agregar a resultado SOLO lo nuevo que se insertó
        $resultado[] = [
...
// armar respuesta según lo que se encontró
if (empty($resultado)) {
    $respuesta = [
        'emails_nuevos' => [],
        'campanas_escaneadas' => $campanasEscaneadas,
        'total_cuentas_procesadas' => $resCuentas->num_rows,
        'mensaje' => 'No se encontraron emails nuevos'
    ];
