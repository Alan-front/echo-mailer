...
$sql = "SELECT e.id, e.id_contacto, e.estado,
               c.secondary_language, c.media_type, c.email, c.name
        FROM enviados e
...
$tradAsunto = [
    'French' => 'Nouveau lancement de',
    'English' => 'New release of',
    'Russian' => 'Новый релиз',
    'Ukrainian' => 'Новий реліз',
    'Korean' => '의 새 출시',
    'Japanese' => 'の新リリース',
    'Spanish' => 'Nuevo lanzamiento de'
];
    ...
    foreach ($mensajesParaEnviar as $index => $item) {
        try {
            ...
            if ($mail->send()) {
                ...
                error_log("Email enviado exitosamente a: " . $item['destinatario']);
            }
